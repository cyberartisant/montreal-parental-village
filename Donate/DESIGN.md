...
The palette leverages a commanding "Institutional Red" (`#ED1B2E`) and a "Trust Blue" (`#007CAF`), balanced by a sophisticated range of neutral surfaces.
...
- **Signature CTAs:** Apply a linear gradient (Top-Left to Bottom-Right) transitioning from `primary` (`#ED1B2E`) to `primary-container` (#e7142a) to add "soul" and dimension to buttons.
...
- **Primary:** `primary` background (`#ED1B2E`) with the signature gradient. `rounded-md` (0.375rem). Use `label-md` in all caps with +0.05em tracking for an authoritative "Action" feel.
- **Secondary:** `secondary` (`#007CAF`) background. Use for supportive, non-emergency actions (e.g., "Join the Community").
...
- **Active State:** On focus, transition the background to `surface-container-lowest` and add a 2px bottom-accent using the `primary` token (`#ED1B2E`).
...
- **Maintain High Contrast:** Ensure all text on `primary` (`#ED1B2E`) or `secondary` (`#007CAF`) backgrounds utilizes `on-primary` (#ffffff) or `on-secondary` (#ffffff).
